class N_gram:
    ...
    def probs_to_neg_log_probs(self, probs_dict):
        ...
        for i in probs_dict.keys():
          neg_log_probs[i] = -np.log(probs_dict[i])
        ### END YOUR CODE HERE ###
    ...
    def filter_adj_counter(self, adj_counter, word_tuple, n):
        ...
        for phrase in adj_counter.keys():
            ### START YOUR CODE HERE ###
            if phrase[:len(word_tuple)] == word_tuple and len(phrase) == n:
    ...
    def p_naive(self, curr_word, prev_phrase, n):
        ...
        try:
            ### START YOUR CODE HERE ###
            prob  = 0 
            b = self.calc_adj_counter(n)
            c = self.filter_adj_counter(b, prev_phrase, n)
            for d in c:
              if d[-1] == curr_word:
                prob = c[d] / sum(c.values())
                print(prob)
            ### END YOUR CODE HERE ###
        except KeyError:
    ...
    def calc_prob_of_sentence(self, sentence_list, n, p_func=p_laplace):
        ...
        """
        ### START YOUR CODE HERE ###
        p = np.exp(-self.calc_neg_log_prob_of_sentence(sentence_list, n, p_func))
        return np.prod(p)
        ### END YOUR CODE HERE ###
