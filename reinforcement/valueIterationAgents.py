...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for v in range(self.iterations):
          c = util.Counter()
          for s in self.mdp.getStates():
            if self.mdp.isTerminal(s):
              c[s] = 0
            else:
              c[s] = self.computeQValueFromValues(s, self.getAction(s))
          self.values = c
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for t in transitions:
          value += t[1]*(self.mdp.getReward(state, action, transitions[0]) + self.discount * self.values[t[0]])
        return value
          ...
          if not pq.isEmpty():
            ...
            if not self.mdp.isTerminal(curr):
              ...
              for action in self.mdp.getPossibleActions(curr):
                values.append(self.computeQValueFromValues(curr, action))
              self.values[curr] = max(values)
